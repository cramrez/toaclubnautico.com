...
?>
    ...
    <?php if(isset($customize['font_family_second'])  && $customize['font_family_second'] != '---'){ ?>
        .testimonial-node-v1 .testimonial-content .quote, .widget.gsc-heading .title-desc
        {
    ...
    <?php if(isset($customize['body_bg_image'])  && $customize['body_bg_image']){ ?>
        body{
            background-image:url('<?php echo \Drupal::service('extension.list.theme')->getPath('gavias_kiamo') .'/images/patterns/'. $customize['body_bg_image']; ?>');
        }
    <?php } ?> 
