    ...
    $( '.isotope-items' ).each(function() {
      ...
      var $el = $( this ),
       ...
       $(window).on('scroll', function () {
        if(reload == 'load' && ( $(window).scrollTop() > ($loop.offset().top - $(window).height()) ) ){
          $loop.isotope();
          $loop.find('.portfolio-filter a').first().trigger('click');
    ...
    chart.each(function() {
      $(this).appear(function() {
        var $this = $(this),
          ...
          lineWidth = ($this.data('bar-line-width')) ? ($this.data('bar-line-width')) : 8
