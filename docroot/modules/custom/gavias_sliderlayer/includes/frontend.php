...
function gavias_sliderlayer_slides($vars){
  ...
  extract($vars);
  ob_start();
    include \Drupal::service('extension.list.module')->getPath('gavias_sliderlayer') . '/templates/frontend/slides.php';
  $output = ob_get_clean();
  return $output;
}
...
function gavias_sliderlayer_slide($vars, $index){
  ...
  extract($vars);
  ob_start();
    include \Drupal::service('extension.list.module')->getPath('gavias_sliderlayer') . '/templates/frontend/slide.php';
  $output = ob_get_clean();
  return $output;
}
...
function gavias_sliderlayer_layer($vars){
  ...
  switch($layer->type){
    ...
    extract($vars);
    ob_start();
      include \Drupal::service('extension.list.module')->getPath('gavias_sliderlayer') . '/templates/frontend/layer.php';
    $output = ob_get_clean();
    return $output;
}
