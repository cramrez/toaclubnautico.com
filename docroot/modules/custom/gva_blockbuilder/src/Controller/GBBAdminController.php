...
class GBBAdminController extends ControllerBase {
  ...
  public function gavias_blockbuilder_edit($bid) {
  	gavias_blockbuilder_init();
    require_once \Drupal::service('extension.list.module')->getPath('gavias_blockbuilder') . '/includes/utilities.php';
    require_once \Drupal::service('extension.list.module')->getPath('gavias_blockbuilder') .'/includes/backend.php';
    ...
    ob_start();
    include \Drupal::service('extension.list.module')->getPath('gavias_blockbuilder') . '/templates/backend/form.php';
  ...
  public function gavias_blockbuilder_export($bid){
  	gavias_blockbuilder_load_includes();
  ...
  public function gavias_upload_file(){
    ...
    if(isset($_FILES['upl']) && $_FILES['upl']['error'] == 0){
      ...
      $file_url = str_replace($base_url, '', \Drupal::service('file_url_generator')->generateAbsoluteString($file_default_scheme. "://gbb-uploads"). '/' .  $image_name . "-{$_id}" . ".{$ext}"); 
  ...
  public function get_images_upload(){
    ...
    $file_url = \Drupal::service('file_url_generator')->generateAbsoluteString($file_default_scheme. "://gbb-uploads"). '/';
    ...
    foreach ($list_file as $key => $file) {
      if(basename($file)){
        $file_url = str_replace($base_url, '', \Drupal::service('file_url_generator')->generateAbsoluteString($file_default_scheme. "://gbb-uploads"). '/' .  basename($file)); 
