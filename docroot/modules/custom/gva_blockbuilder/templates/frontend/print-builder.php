<div class="gbb-row-wrapper">
  ...
  <?php 
    if( (isset($row_attr['bg_video_mp4']) && !empty($row_attr['bg_video_mp4']))
    || (isset($row_attr['bg_video_webm']) && !empty($row_attr['bg_video_webm'])) 
    || (isset($row_attr['bg_video_ogv']) && !empty($row_attr['bg_video_ogv'])) ){ ?>
    <?php
      $bg_video_mp4 = $row_attr['bg_video_mp4'];
      $bg_video_webm = $row_attr['bg_video_webm'];
      $bg_video_ogv = $row_attr['bg_video_ogv'];
    ?>
      <video data-video-element="" data-ref="background-videos" class="bg-video-html visual false" autoplay="autoplay" muted="" poster="<?php echo (isset($row_attr['bg_image']) && $row_attr['bg_image']) ? $base_url . $row_attr['bg_image'] : ''  ?>" data-icon="play" loop="" playsinline="">
          <?php 
            if($bg_video_mp4){ 
              echo '<source src="' . $base_url . $bg_video_mp4 . '" type="video/mp4">';
            }
          ?>
          <?php 
            if($bg_video_webm){ 
              echo '<source src="' . $base_url . $bg_video_webm . '" type="video/webm">';
            }
          ?>
          <?php 
            if($bg_video_ogv){ 
              echo '<source src="' . $base_url . $bg_video_ogv . '" type="video/ogv">';
            }
          ?>
