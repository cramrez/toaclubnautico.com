...
class gavias_sc{
  ...
  public function gsc_load_shortcodes($backend=true){
    if( empty($this->gv_shortcodes) ){
      ...
      if(file_exists($theme_path . '/gva_elements/elements.php')){
        
        $shortcodes = [];
        if(function_exists('gavias_blockbuilder_get_elements')) $shortcodes = gavias_blockbuilder_get_elements();
        foreach( $shortcodes as $sc ){
          $sc_path = '';
          if(file_exists($theme_path . '/gva_elements/' . $sc . '.php')){
            $sc_path = $theme_path . '/gva_elements/' . $sc . '.php';
          }
          if($sc_path){
            $class = $sc;
      ...
      //Load elements from gavias_elements modules (financial, charityplus ,sanbro, carina)
        ...
        foreach( $shortcodes as $sc ){
          $sc_path = '';
          if(file_exists(\Drupal::service('extension.list.module')->getPath('gavias_elements') . '/elements/' . $sc . '.php')){
            $sc_path = \Drupal::service('extension.list.module')->getPath('gavias_elements') . '/elements/' . $sc . '.php';
          }
          if($sc_path){
            $class = $sc;
            $_class = '\\Drupal\gavias_blockbuilder\shortcodes\\'.$class;
  ...
  public  function row_opts(){
   return array(
      ...
      array(
            'id'        => 'info_1',
            'type'      => 'info',
            'desc'      => 'Background Video HTML'
          ),
          array(
            'id'         => 'bg_video_mp4',
            'type'       => 'upload',
            'title'      => t('Background Video .mp4'),
            'class'       => 'width-1-3 no-preview',
          ),
          array(
            'id'         => 'bg_video_webm',
            'type'       => 'upload',
