...
function gavias_admin_element( $item_std, $item = false, $column_id = false, $row_id = false) {
	...
	ob_start();
  	include \Drupal::service('extension.list.module')->getPath('gavias_blockbuilder') . '/templates/backend/admin-element.php';
...
function gavias_admin_row( $item_std, $row_std, $column_std, $row = false, $row_id = false ) {
	...
	ob_start();
  	include \Drupal::service('extension.list.module')->getPath('gavias_blockbuilder') . '/templates/backend/admin-row.php';
  	$content = ob_get_clean();
	print $content;		
}
...
function gavias_admin_column( $item_std, $column_std, $column = false, $column_id = false,  $row_id = false ) {
	...
	ob_start();
  	include \Drupal::service('extension.list.module')->getPath('gavias_blockbuilder') . '/templates/backend/admin-column.php';
  	$content = ob_get_clean();
	print $content;		
}
...
function gavias_blockbuilder_admin($pid) {
	...
	ob_start();
  	include \Drupal::service('extension.list.module')->getPath('gavias_blockbuilder') . '/templates/backend/admin-builder.php';
  	$content = ob_get_clean();
	print $content;
}
